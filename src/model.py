...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, stride, dilation, downsample):
        ...
        mid_ch = out_ch // self.bottleneck_expansion
        self.conv1x1_in = ConvBnReLU(in_ch, mid_ch, 1, stride, 0, 1, True)
        self.conv3x3 = ConvBnReLU(mid_ch, mid_ch, 3, 1, dilation, dilation, True)
        self.conv1x1_out = ConvBnReLU(mid_ch, out_ch, 1, 1, 0, 1, False)
        self.shortcut = (
...
class DeepLabV3(nn.Sequential):
    ...
    def __init__(self, n_classes, n_blocks, atrous_rates, multi_grids, output_stride):
        ...
        self.add_module("layer4", ResNetLayer(n_blocks[2], ch[3], ch[4], s[2], d[2]))
        self.add_module(
            "layer5", ResNetLayer(n_blocks[3], ch[4], ch[5], s[3], d[3], multi_grids)
        )
